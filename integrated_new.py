...
# # filename = blockPath = r"\Users\maryz\EEG-Video\bin_files\ear3.31.25_1.bin"
# BLOCK_PATH = r"\Users\maryz\EEG-Video\SubjectG-250331-165322"
# ADCres = 12  # ADC resolution in bits
# fsBionode = 6250  # Sampling frequency in Hz
# channel = 2  # EEG channel to analyze
...
# rawCha = raw[1:].astype(np.float32)
...
# print(f"[DEBUG] Filtered EEG shape: {filtered.shape}, Time shape: {time.shape}")
...
# print(f"[DEBUG] eeg_alpha shape: {eeg_alpha.shape}")
# print(f"[DEBUG] eeg_alpha first 5 samples: {eeg_alpha[:5]}")
...
# # ax_alpha.set_ylim(2,40) #old for neuropulse
# alpha_min = np.min(alpha_power[np.isfinite(alpha_power)])
# alpha_max = np.max(alpha_power[np.isfinite(alpha_power)])
...
#     # === EEG update ===
#     eeg_mask = (time >= current_time) & (time <= current_time + window_sec)
#     print(f"[DEBUG] EEG mask active: {np.sum(eeg_mask)} samples at time {current_time:.2f}s")
...
#     try:
#         alpha_mask = (time_sec_alpha >= current_time) & (time_sec_alpha <= current_time + window_sec)
#         print(f"[DEBUG] Alpha mask active: {np.sum(alpha_mask)} samples from {current_time:.2f} to {current_time+window_sec:.2f}")
