...
blockPath = '/Users/alessandroascaniorsini/Downloads/SubjectG-250331-172451'

# Retrieve sampling rate from TDT file
init_block = read_block(blockPath, store='EEGw', channel=[channel], t1=0, t2=0)
fsBionode = init_block.streams['EEGw'].fs
# Recompute original-sampling window parameters based on actual fsBionode
alpha_window_samples_orig = int(window_size * fsBionode)

# VIDEO CONFIG
video_path = "/Users/alessandroascaniorsini/Desktop/1724.mov"
...
# EEG PROCESSING
def preprocess_eeg():
    # Load TDT block and get the EEG stream
    block = read_block(blockPath, store='EEGw', channel=[channel], t1=0)
    # block.streams['EEGw'].data has shape (1, n_samples)
    raw_data = block.streams['EEGw'].data
    # Update sampling rate from TDT metadata
    global fsBionode
    fsBionode = block.streams['EEGw'].fs
    # Handle data shape: if multi-channel, select channel 0; if single-vector, use directly
    if raw_data.ndim > 1:
        rawCha = raw_data[0]
    else:
        rawCha = raw_data
    ...
    eeg_filtered = signal.sosfiltfilt(sos_bp, rawCha)
    # Downsample
